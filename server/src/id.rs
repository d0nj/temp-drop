...
pub fn new_id() -> String {
    ...
    bytes.iter().map(|b| ALPHABET[*b as usize % 62] as char).collect()
}
...
impl std::fmt::Display for NameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::TooLong => write!(f, "filename exceeds 255 chars"),
        }
    }
}
...
/// Strip C0/C1 control chars, trim, cap at 255 chars. Empty after cleaning -> Err.
...
mod tests {
    ...
    fn sanitize_strips_control_and_trims() {
        ...
        assert_eq!(sanitize_name(&"a".repeat(255)).unwrap().len(), 255);
    }
